...
## Parse text-shadow (e.g., "2px 2px 4px #000000" or "1px 1px rgba(0,0,0,0.5)").
## Supports: offset-x offset-y [blur-radius] [color]
## Returns a Dictionary with shadow properties.
static func parse_text_shadow(value: String) -> Dictionary:
	value = value.strip_edges()
...
## Parse outline shorthand (e.g., "2px solid #ff0000" or "none").
## Returns a Dictionary with "width", "style", and "color" keys.
static func parse_outline(value: String) -> Dictionary:
	value = value.strip_edges()
